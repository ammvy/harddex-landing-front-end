...
export function TduSelector({
  ...
  const profiles = ["CREATIVE", "DEV", "GAMER", "MOBILE", "PRO", "STUDY"];
  return <div className="p-4 bg-muted/20">
      <div style={{
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
        ...
        return <button key={profileId} type="button" onClick={() => {
          if (!disabled) {
            onTduChange(profileId);
          }
        }} disabled={disabled} className={`flex items-center gap-3 p-2 border transition-all duration-150 cursor-pointer text-left rounded-none ${active ? "border-primary bg-primary/5 shadow-[0_0_12px_rgba(58,112,244,0.15)] scale-[1.02]" : "border-foreground/10 hover:border-primary/50 hover:bg-muted/10"} ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}>
