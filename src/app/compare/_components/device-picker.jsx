...
export function DevicePicker({
  ...
  return <div className="relative flex-1">
      <button type="button" onClick={() => onOpenChange(!open)} className="w-full text-left border border-foreground px-4 py-3 flex items-center justify-between gap-3 hover:border-primary transition-colors duration-100 bg-background text-foreground cursor-pointer">
        ...
        <ChevronDown size={14} strokeWidth={1.8} className={`transition-transform duration-150 ${open ? "rotate-180" : ""}`} />
      </button>
      <AnimatePresence>
          ...
          return <button key={d.id} type="button" onClick={() => {
            ...
            onOpenChange(false);
                ...
                background: d.accent
                  ...
                  {active && <Check size={14} strokeWidth={2.2} className="text-primary" />}
                </button>;
        })}
          </motion.div>}
      </AnimatePresence>
