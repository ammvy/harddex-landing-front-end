...
export default function AskMouse({
  ...
  return <section className="mt-14">
      ...
      <div className="border border-foreground bg-background divide-y divide-foreground/10">
        ...
        <TduSelector selectedTdu={tdu} onTduChange={setTdu} disabled={isAnimating} />
        ...
        <div className="flex flex-col md:flex-row items-start gap-6 p-6">
          ...
          {/* Chat Bubble */}
          <ChatBubble state={state} response={response} error={error} tdu={tdu} deviceA={deviceA} deviceB={deviceB} onReset={reset} />
        </div>

        {/* Ask Opinion Button Area */}
        <div className="flex justify-end p-4 bg-muted/10">
          <button type="button" onClick={handleAsk} disabled={isAnimating} className={`w-full sm:w-auto flex justify-center items-center gap-2 bg-primary text-primary-foreground hover:cursor-pointer hover:bg-transparent hover:border-primary hover:text-primary border-transparent border-solid border-2 p-3 px-6 transition-all duration-150 uppercase tracking-wider font-bold text-[12px] ${isAnimating ? "opacity-50 cursor-not-allowed" : ""}`} style={{
          fontFamily: "'Space Mono', monospace"
        }}>
            <span>
              {state === "loading" || state === "streaming" ? "Analisando..." : state === "done" ? "Analisar Novamente" : "Pedir Opinião"}
            </span>
