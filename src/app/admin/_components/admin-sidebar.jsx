...
import { LayoutGrid, Package, Factory, Cpu, Tag, Award, Users, X } from "lucide-react";
...
export default function AdminSidebar({
  ...
  const navList = <nav className="flex flex-col" style={{
    fontFamily: "'Space Mono', monospace"
  }}>
      {NAV_ITEMS.map(n => {
      const active = pathname === n.href;
      return <Link key={n.href} href={n.href} onClick={onClose} className={`flex items-center gap-3 px-5 py-4 border-b border-border/40 uppercase tracking-widest text-[11px] text-left transition-colors duration-100 ${active ? "bg-primary text-primary-foreground border-primary" : "hover:text-primary text-foreground"}`}>
            {n.icon}
            {n.label}
          </Link>;
    })}
        ...
        {isOpen && <div className="lg:hidden fixed inset-0 z-40">
          ...
          ease: [0.22, 1, 0.36, 1]
              ...
              fontFamily: "'Space Mono', monospace"
                ...
                <button onClick={onClose} className="hover:text-primary text-foreground cursor-pointer">
