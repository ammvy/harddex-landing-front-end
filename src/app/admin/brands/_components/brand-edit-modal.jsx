...
export default function BrandEditModal({
  ...
  return <Modal title={brand.id ? "Editar marca" : "Nova marca"} onClose={onClose} footer={<>
      ...
      <Field label="Nome">
        ...
        ...draft,
        name: e.target.value
      })} className="bg-input-background border border-border px-3 py-2.5 outline-none text-[13px] focus:border-primary transition-colors duration-100 text-foreground placeholder:text-muted-foreground/50 w-full" placeholder="Ex: NVIDIA" />
      </Field>
    </Modal>;
}