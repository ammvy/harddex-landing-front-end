...
export default function BrandsTable({
  ...
  return <div>
      <SectionHead kicker="Catálogo" title="Marcas" action={<button onClick={() => setEditingBrand(emptyBrand())} style={{
      ...
      <div className="border border-border overflow-x-auto bg-background">
        <table className="w-full min-w-[640px] text-left">
          ...
          <tbody className="divide-y divide-border/40 text-foreground">
            {brands.map(b => <tr key={b.id} className="hover:bg-input-background/40 transition-colors duration-75">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1.5">
                    ...
                    <IconBtn title="Excluir" danger onClick={() => setConfirmDel(b)}>
      ...
      <AnimatePresence>
        {editingBrand && <BrandEditModal brand={editingBrand} onClose={() => setEditingBrand(null)} onSave={saveBrand} />}

        {confirmDel && <Modal title="Excluir marca" onClose={() => setConfirmDel(null)} footer={<>
          ...
          fontFamily: "'Space Mono', monospace"
                ...
                <button onClick={() => deleteBrand(confirmDel.id)} style={{
          fontFamily: "'Space Mono', monospace"
