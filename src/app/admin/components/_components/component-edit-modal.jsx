...
export default function ComponentEditModal({
  ...
  return <Modal title={component.id ? "Editar componente" : "Novo componente"} onClose={onClose} footer={<>
      ...
      <Field label="Nome">
        ...
        ...draft,
        name: e.target.value
      })} className="bg-input-background border border-border px-3 py-2.5 outline-none text-[13px] focus:border-primary transition-colors duration-100 text-foreground placeholder:text-muted-foreground/50 w-full" placeholder="Ex: Chip Gráfico AD104" />
      </Field>

      <Field label="Especificação (JSON)">
        <textarea value={specText} onChange={e => handleSpecChange(e.target.value)} className={`bg-input-background border ${jsonError ? "border-destructive focus:border-destructive" : "border-border focus:border-primary"} px-3 py-2.5 outline-none text-[12px] font-mono transition-colors duration-100 text-foreground w-full min-h-[100px]`} placeholder='{\n  "Cores": "8",\n  "Clock": "3.5GHz"\n}' />
      ...
      })} className="bg-input-background border border-border px-3 py-2.5 outline-none text-[13px] focus:border-primary transition-colors duration-100 text-foreground placeholder:text-muted-foreground/50 w-full min-h-[80px]" placeholder="Descrição do componente..." />
      </Field>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Produto Pai">
          <select value={draft.productId} onChange={e => setDraft({
          ...draft,
        ...
        <Field label="Fabricante">
          <select value={draft.manufacturerId || ""} onChange={e => setDraft({
          ...draft,
          manufacturerId: e.target.value ? Number(e.target.value) : null
        })} style={{
          fontFamily: "'Space Mono', monospace"
        }} className="bg-input-background border border-border px-3 py-2.5 outline-none text-[11px] focus:border-primary transition-colors duration-100 text-foreground uppercase tracking-wider w-full">
            <option value="">Nenhum</option>
            {manufacturers.map(m => <option key={m.id} value={m.id}>
                {m.name}
              </option>)}
          </select>
        </Field>
